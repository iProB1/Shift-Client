...
bool ShowMenu = true; // auto opens if its false you need click INSERT for open or whatever key 
namespace Process {
	DWORD ID;
	HANDLE Handle;
	HWND Hwnd;
...
HRESULT APIENTRY MJEndScene(IDirect3DDevice9* pDevice) {
	if (!ImGui_Initialised) {
        ...
        io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\Arial.ttf", 13); // font
	...
	if (ShowMenu && ImGui::Begin("Shift Client", &ShowMenu)) { // imgui menu begins
		Tab::Main(); // your menu
		ImGui::End(); // imgui ends
	}
